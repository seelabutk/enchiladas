;(function ($, window){
    ...
    Tapestry.prototype.animate = function()
    {
        ...
        for (var i = 0; i < this.keyframes.length - 1; i++)
        {
            ...
            {
                var quat = this.camera.slerp(this.keyframes[i], this.keyframes[i + 1], j).elements;
                this.camera.rotateFromQuaternion(quat);
                var path = this.render(0, undefined, true);
                path += "onlysave," + counter.toString();
                counter++;
                var temp = new Image();
                temp.src = path;
            }
    ...
    Tapestry.prototype.setup_handlers = function()
    {
        ...
        $(this.element).on("mousedown", function(event){
            if (event.which == 3)
            {
                ...
                self.keyframes.push(self.camera.Quat.map(function(x){return parseFloat(x);}));
                return
            }
